...
/client/verb/callproc_target(atom/A in range(world.view))
	set category = "Debug"
...
/client/proc/callproc_targetpicked(hastarget, datum/target)
	...
	if(hastarget)
		if(!target)
			usr << "Your callproc target no longer exists."
			return
		world << "[key_name(src)] called [target]'s [procname]() with [arguments.len ? "the arguments [list2params(arguments)]" : "no arguments"]."
		if(arguments.len)
			returnval = call(target, procname)(arglist(arguments))
		else
			returnval = call(target, procname)()
	else
		world << "[key_name(src)] called [procname]() with [arguments.len ? "the arguments [list2params(arguments)]" : "no arguments"]."
